...
client.on("guildMemberAdd", (member) => {
  ...
  member.guild.fetchInvites().then(newinvites => {
    ...
    var token = tokens.find(token => invite.channel.parent.name.includes(token));
    if (!token) { return null; }
    member.guild.roles.fetch().then(roles => {
      var visitorrole = roles.cache.find(role => role.name === "Visitor");
      var localvisitorrole = roles.cache.find(role => role.name.startsWith("Visitor") && role.name.endsWith(token));
      member.roles.add(visitorrole);
      member.roles.add(localvisitorrole);
      crewchannel = invite.channel.parent.children.find(channel => channel.name.startsWith(token.toLowerCase()) && channel.name.endsWith("crew"));
